...
public class FakeArticleRepository {
	...
	static {
		...
		articlesDb.put(2L, new Article(2L, "Les furtifs", 25.50, "Un chef d'oeuvre d'Alain Damasio", LocalDate.now(),
				Emballage.SANS));
		articlesDb.put(3L, new Article(3L, "Adidas d'occasion", 17.45, "Usées", LocalDate.now(), Emballage.CARTON));
		articlesDb.put(4L,
				new Article(4L, "Ramette", 10.99, "Papier de bonne qualité", LocalDate.now(), Emballage.PAPIER));
	...
	public Article save(Article a) {
		if (a.getId() == 0) {//Création
			a.setId(nextId);
			articlesDb.put(nextId, a);
			return articlesDb.get(nextId++);
		} else {//Modification (par écrasement)
			articlesDb.put(a.getId(), a);
			return articlesDb.get(a.getId());
		}
	}
